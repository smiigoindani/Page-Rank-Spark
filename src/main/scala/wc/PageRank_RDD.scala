...
object PageRank_RDD {
  ...
  def main(args: Array[String]) {
    ...
    for (i <- 1 to 10) {
      //println("iteration")
      //println(i)
      val MergeRank = graph.join(ranks).flatMap(tuple => if (tuple._1 % k == 1) List((tuple._1, 0.0), tuple._2) else List(tuple._2)) 
      val MergeRank_g = MergeRank.groupByKey().map(tuple => (tuple._1, tuple._2.sum))
      val delta_filter = MergeRank_g.filter(tuple => tuple._1 == 0).map(tuple => tuple._2).collect
      val delta = delta_filter(0)
      ranks = MergeRank_g.map(tuple => if (tuple._1 != 0) (tuple._1, tuple._2 + delta/(k*k)) else (tuple._1, 0.0))
